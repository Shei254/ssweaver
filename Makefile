...
COMMIT_HASH         = $(shell git rev-parse --short HEAD)go
...
.PHONY: lib
lib: $(SOURCES) ## build ssweaver library
	go build ./...

.PHONY: ssweaver
ssweaver: $(SOURCES) ## build ssweaver binary
	go build -ldflags "-X main.version=$(VERSION) -X main.commit=$(COMMIT_HASH)" -o bin/ssweaver ./cmd/ssweaver
...
.PHONY: build
build: $(SOURCES) lib ssweaver eskip webhook routesrv ## build library and all binaries

build.linux.static: ## build static linux binary for amd64
	GOOS=linux GOARCH=amd64 CGO_ENABLED=0 go build -o bin/ssweaver -ldflags "-extldflags=-static -X main.version=$(VERSION) -X main.commit=$(COMMIT_HASH)" ./cmd/ssweaver

build.linux.arm64: ## build linux binary for arm64
	GOOS=linux GOARCH=arm64 CGO_ENABLED=0 go build -o bin/ssweaver -ldflags "-X main.version=$(VERSION) -X main.commit=$(COMMIT_HASH)" ./cmd/ssweaver

build.linux.armv7: ## build linux binary for arm7
	GOOS=linux GOARCH=arm GOARM=7 CGO_ENABLED=0 go build -o bin/ssweaver -ldflags "-X main.version=$(VERSION) -X main.commit=$(COMMIT_HASH)" ./cmd/ssweaver

build.linux:
	GOOS=linux GOARCH=amd64 CGO_ENABLED=0 go build -o bin/ssweaver -ldflags "-X main.version=$(VERSION) -X main.commit=$(COMMIT_HASH)" ./cmd/ssweaver

build.darwin.arm64: ## build osx binary for arm64
	GOOS=darwin GOARCH=arm64 CGO_ENABLED=0 go build -o bin/ssweaver -ldflags "-X main.version=$(VERSION) -X main.commit=$(COMMIT_HASH)" ./cmd/ssweaver

build.darwin: ## build osx binary for amd64
	GOOS=darwin GOARCH=amd64 CGO_ENABLED=0 go build -o bin/ssweaver -ldflags "-X main.version=$(VERSION) -X main.commit=$(COMMIT_HASH)" ./cmd/ssweaver

build.windows: ## build windows binary for amd64
	GOOS=windows GOARCH=amd64 CGO_ENABLED=0 go build -o bin/ssweaver -ldflags "-X main.version=$(VERSION) -X main.commit=$(COMMIT_HASH)" ./cmd/ssweaver

.PHONY: install
install: $(SOURCES) ## install ssweaver and eskip binaries into your system
	go install -ldflags "-X main.version=$(VERSION) -X main.commit=$(COMMIT_HASH)" ./cmd/ssweaver
